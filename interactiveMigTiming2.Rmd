...
```{r processing}
numericInput('ey', 'Elk year index', 1,
              min = 1, max =  1000 )

selectedData <- reactive({
  ind <- bursts %>% filter(elkYear == unique(elkYear)[input$ey])
...
selectedInd <- reactive({unique(bursts$elkYear)[input$ey]})
...
renderPlotly({
     plot_ly(selectedData(), x = ~date, y = ~distance) %>%
      ...
      layout(title=paste(selectedInd(), "classified as", strategy()))
